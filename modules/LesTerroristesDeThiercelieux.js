...
class LesTerroristesDeThiercelieux extends BaseModule {
	...
	initChannels() {
		for (var conf of this.data.channels.config) {
			this.data.channels[conf.id] = this.data.guild.channels.find((found) => (found.id == conf.discord_id))
			if (!this.data.channels[conf.id]) {
	...
	commandStart(message) {
		if (this.data.members.length < 3) {
			this.send("Y'a même pas 3 personnes, ça va être triste, même pas je lance :cry:")
			this.reset()
		} else {
...
// INIT AND MISC
	...
	initData() {
		this.data = {
			...
			members: [],
			commands: [
				{ id: 'init', method: "commandInit", needs_init: false, needs_start: false, authorized_channels: ['lobby', 'history'] },
				...
				{ id: 'test', method: "commandTest", needs_init: false, needs_start: false, authorized_channels: ['lobby'] },
			],
			command: {
				args: null,
				name: null,
